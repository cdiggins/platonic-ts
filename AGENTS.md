...
| `packages/hooks` | All guard and hook logic: event codec, staging/dialect rules (pure, tested), entry points for the PreToolUse and git pre-commit guards. `.githooks/` and `.claude/settings.json` are logic-free launchers only — rules never live there (see `docs/decisions/2026-08-23-hooks-logic-lives-in-packages-hooks.md`). |
