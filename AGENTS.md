...
| `packages/backlog` | Parse/render/load backlog markdown items in `backlog/`; allocates `BL-NNNN` ids (`npm run backlog:next-id -- <slug>`) and validates them (`npm run backlog:validate`). Never pick an id by scanning for the highest number — see `docs/backlog-id-allocation-2026-08-23.md`. Also owns the repo's only documentation generator (`src/docsgen.ts` + `src/docsgenIo.ts`, `npm run docs:regen`): it fills `<!-- BEGIN GENERATED -->` blocks in README.md, docs, and each `packages/*/src/INDEX.md`. A second doc generator must not be built elsewhere. |
| `packages/check` | The check gate — `npm run check` runs typecheck, lint, ratchet, tests, backlog ids, import boundaries, and doc-block freshness in `src/run.ts`, stopping at the first failure. Gates only: it reports, it never writes source or docs, and it must never import `packages/codemap`. |
...
subfolder of one that holds source files) carries one. Its opening paragraph is hand-written;
its file table is generated by `npm run docs:regen` from each file's PS-057 purpose comment,
and `npm run check` fails when the committed table is not what regeneration would write. Add
a source file without that comment and the gate stops you.
